...
if (!('webkitSpeechRecognition' in window)) {  // 如果找不到 window.webkitSpeechRecognition 這個屬性
  ...
  recognition.interimResults = true; // 如果設定為 true，表示在我們講話的當下就會即時辨識，不然就會在一段話結束之後，才會開始辨識。
  ...
  recognition.onresult = function(event) { // 辨識有任何結果時
    ...
    if (final_transcript.trim().length > 0){ // 如果有最終辨識文字    	
        //textBox.value = word ;// 顯示最終辨識文字
        StrToHandle_talk(final_transcript);
   ...
   var form = document.createElement("form");
      ...
      form.setAttribute("action","Handle/SpeechApi_handle.php");
        var str_input = document.createElement("input");
          ...
          str_input.setAttribute("value",string);
